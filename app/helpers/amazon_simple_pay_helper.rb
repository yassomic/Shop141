module AmazonSimplePayHelper
  ...
  def amazon_simple_pay_form_tag(options = {}, &block)
    ...
    output << content
    output.safe_concat(hidden_field_tag('accessKey', ENV['ACCESS_KEY_ID']))
    output.safe_concat(hidden_field_tag('amazonPaymentsAccountId', ENV['AMAZON_PAYMENTS_ACCOUNT_ID']))
